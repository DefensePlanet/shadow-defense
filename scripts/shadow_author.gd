...
func _draw() -> void:
	...
	# === SPRITE RENDERING (split-body animated — dark & ethereal) ===
	if sprite_texture:
		...
		# --- LEGS (bottom half): planted, ethereal stillness ---
		var leg_breathe = 1.0 + sin(_time * 1.8) * 0.006  # very subtle
		var leg_sway = sin(_time * 1.0) * 0.007  # barely noticeable
		var leg_brace = 0.0
		if _attack_anim > 0.0:
			leg_brace = sin(_attack_anim * PI) * 0.014
		...
		# --- TORSO (top half): active, dark ethereal sway ---
		var breathe_scl = 1.0 + sin(_time * 1.8) * 0.033
		var sway_rot = sin(_time * 1.0) * 0.042

		# Aim lean: torso twists toward target (ethereal)
		var aim_lean = sin(aim_angle) * 0.068
		...
		if _attack_anim > 0.0:
			...
			atk_scl = Vector2(1.0 + sq * (0.15 + float(upgrade_tier) * 0.025), 1.0 - sq * (0.10 + float(upgrade_tier) * 0.02))
