...
func _draw() -> void:
	...
	# === SPRITE RENDERING (split-body animated — elegant & sinister) ===
	if sprite_texture:
		...
		# --- LEGS (bottom half): planted, elegant stillness ---
		var leg_breathe = 1.0 + sin(_time * 1.5) * 0.004  # very subtle
		var leg_sway = sin(_time * 0.7) * 0.006  # barely noticeable
		var leg_brace = 0.0
		if _attack_anim > 0.0:
		...
		# --- TORSO (top half): active, sinister elegance ---
		var breathe_scl = 1.0 + sin(_time * 1.5) * 0.024
		var sway_rot = sin(_time * 0.7) * 0.033
