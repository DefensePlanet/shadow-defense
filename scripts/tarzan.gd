...
func _draw() -> void:
	...
	# === SPRITE RENDERING (split-body animated — wild & powerful) ===
	if sprite_texture:
		...
		# --- LEGS (bottom half): planted, powerful stance ---
		var leg_breathe = 1.0 + sin(_time * 2.5) * 0.006  # very subtle
		var leg_sway = sin(_time * 1.4) * 0.009  # barely noticeable
		var leg_brace = 0.0
		if _attack_anim > 0.0:
			leg_brace = sin(_attack_anim * PI) * 0.020  # braces hard on smash
		...
		# --- TORSO (top half): active, wild smash ---
		var breathe_scl = 1.0 + sin(_time * 2.5) * 0.036
		var sway_rot = sin(_time * 1.4) * 0.053

		# Aim lean: torso twists toward target (wild)
		var aim_lean = sin(aim_angle) * 0.075

		# Attack recoil + squash-stretch (big forward smash)
		...
		if _attack_anim > 0.0:
			...
			atk_scl = Vector2(1.0 + sq * 0.23, 1.0 - sq * 0.15)
			# Torso snap on attack
			sway_rot += sin(_attack_anim * PI * 2.0) * 0.06
